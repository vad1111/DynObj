...
namespace Динамические
{
    public class АдресУРегистратора : Адрес
    {
        ...
        public override РАМОбъект АдресВКучеПамяти() // это для исходящей связи .  должна озвращаться Связь а не Примитив
        {
            if (АдресРегистратораВкуче == null) АдресРегистратораВкуче = (РегистраторАдресов) АдресРегистратора.АдресВКучеПамяти();
            return  АдресРегистратораВкуче.ДайАдресВкуче(НомерУРегистратора);
        }

        public bool Зарегистрировать(РАМОбъект объект, Guid пароль) // это для входящейсвязи 
        {
            АдресРегистратораВкуче = (РегистраторАдресов) АдресРегистратора.АдресВКучеПамяти();
    ...
    public class СвязьПамять: Связь //это канал связи или сеанс чтобы скрыть способ реализации связи и описать ее активности "имеют" связи, связи связываются только с друг с другом 
    {
        ...
        //Входящие сообщения 
        public РАМОбъект СообщениеВходящее { get; private set; }
        ...
        {
            var регистратор = (РегистраторАдресов) адресРегистратора.АдресВКучеПамяти() ;
            Пароль = Guid.NewGuid();
        ...
        {
            var рег = (РегистраторАдресов) СвойАдресУРегистратора.АдресРегистратора.АдресВКучеПамяти();
        ...
        { 
            АдресВКучеУдаленнаяСвязь = (СвязьПамять) АдресУдаленнойСвязи.АдресВКучеПамяти(); // это для связи в памяти 
        ...
        public override void ПередайСообщение(РАМОбъект сообщение)
        {
        ...
        public override РАМОбъект ПередайСообщениеСОтветом(РАМОбъект сообщение)
        {
            if (АдресВКучеУдаленнаяСвязь == null) УстановитьСвязь();
        ...
        public override void ПолучиСообщение( РАМОбъект сообщение)
        {
        ...
        public override РАМОбъект ПолучиСообщениеСОтветом( РАМОбъект сообщение)
        {
            //if (ВходнаяСвязь == АдресВКучеУдаленнаяСвязь) // если полученный адрес соответсвует запомненому, внутри можно попроще 
