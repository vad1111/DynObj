...
{
    public class КлассCSharp : РАМОбъект //Описание Структуры типа База данных = фиксированный список полей, 
    ...
    {
        public СтрокаЮникода Имя; // ссылка это " эта группа имеет имеет имя ИИИИ 
        ...
        public static РАМОбъект  СоздатьТип( СтрокаЮникода ИмяКласса)
        {
        ...
        static КлассCSharp()
        {
            ...
            группаТипов.Добавить(СтрокаЮникода.Создать("Тип")); // это группа адресов
    ...
    public class Класс
    {
        ...
        public static void Тест()
        {
            ...
            класс.Свойства[0]= new СтрокаЮникода("Имя класса");
            var поле1 = new ГруппаОбъектов();
            класс.Свойства[1] = поле1;
            поле1[0]= new СтрокаЮникода("это Поле");
            поле1[1] = new СтрокаЮникода("имяПоля1");
            поле1[2] = new СтрокаЮникода("ТипаЗначения"); //может быть не опрееделен, может иметь группу типов
            поле1[3] = new СтрокаЮникода("ЗначениеПоля");

            var класс2= new ГруппаОбъектов();// это класс
            класс2[0]= new СтрокаЮникода("Это тип объектов"); // или адрес на строку что это Тип. Описание типа это группа которая может иметь группу полей и группу методов
            класс2[1] = new СтрокаЮникода("Имя типа (Целое)"); // имеет имя 
            класс2[2] = поле1; // имеет поле

            var тип = new ГруппаОбъектов();
            var адресОписанияТипов = Хранилище.Добавить(тип);
            тип[0]= new СтрокаЮникода("Это описание типов");
            тип[1] = new СтрокаЮникода("имеет список полей (создать ГруппуОбъектов с именем список полей)"); ; // это список полей
            ((ГруппаОбъектов)тип[1])[0]= new СтрокаЮникода("это список полей");
            тип[2] = new ГруппаОбъектов(); // это список методов
            ((ГруппаОбъектов)тип[2])[0] = new СтрокаЮникода("это список методов");
            ...
            локальныйСписокТипов[0] = new СтрокаЮникода("Это локальный список типов");
            ...
            матрицаТипа[0] = new СтрокаЮникода("это матрица тип");
            матрицаТипа[1] = new СтрокаЮникода("Имя типа");
            матрицаТипа[2] = new ГруппаОбъектов(); ; // это список полей
                        ((ГруппаОбъектов)матрицаТипа[2])[0] = new СтрокаЮникода("это список полей");
            матрицаТипа[3] = new ГруппаОбъектов(); // это список методов
                        ((ГруппаОбъектов)матрицаТипа[3])[0] = new СтрокаЮникода("это список методов");
            ...
            На_ИмяТипа(класс4, new СтрокаЮникода("Бар")); // 
            ...
            {
                [0] = new СтрокаЮникода("метод"),
                [1] = new СтрокаЮникода("Имя метода"),
                ...
                [3] = new РАМОбъект(), // определение типа результата
            ...
            действие.Выполнить(new object[0]);// сдесь в качестве параметра надо задавать список аргументов
            ...
            {
                [0] = new АппаратноеДействие() { Метод = typeof(Класс).GetMethod("СоздатьТип"), Метка = new СтрокаЮникода("Создать тип")},
        ...
        public static void ВыполнитьМетод(ГруппаОбъектов метод)
        {
            ...
            метод[3]= (РАМОбъект) действия.Выполнить(new object[0]);
        ...
        public static Адрес СоздатьТип(string имя)
        {
            var Тип = new ГруппаОбъектов
            {
                [0] = new СтрокаЮникода("тип"), //это общее для группы схожих объектов , если объекту задать вопрос (провести измерение) объект переадресует этот вопрос к этому адресу
                [1] = new СтрокаЮникода(имя),      // это по сути описание словосочетания и одновременно матрица для экземпляров типа (объектов)
                [2] = new ГруппаОбъектов {[0] = new СтрокаЮникода("список полей") }, // это список полей "имеет" эта группа типа "список" т.е. содержит однотипные объекты типа "поле", т.е. тип "поле" можно убрать
                [3] = new ГруппаОбъектов {[0] = new СтрокаЮникода("список методов") } // это список методов "имеет"
            };
        ...
        // аналог toString()
        public static РАМОбъект ЧтоЭто(ГруппаОбъектов объект, ГруппаОбъектов интерпритация) 
            ...
            //фиксация структуры и связанных с ней методов создают новую модель реальности (например объектов)
        {
            ...
            {
                [0] = new СтрокаЮникода("Что это?"), // это описание или запрос от кого пришло, что пришло.
                [1] = new СтрокаЮникода("Это "),
                [2] = (СтрокаЮникода) объект[0]   // ответ "это тип" , вопрос чего? имя типа? с другой стороны это группа объектов 
            };
        ...
        public static void На_ИмяТипа(ГруппаОбъектов класс, СтрокаЮникода имяТипа)
        {
            if( класс[0]== new СтрокаЮникода("тип")) // не корректое сравнение исправить, присваивает имя только типам 
           класс[1]=имяТипа;
        }
        public static СтрокаЮникода Дай_ИмяТипа(ГруппаОбъектов класс)
        {
            return (СтрокаЮникода) класс[1] ;
        }
        public static ГруппаОбъектов СоздатьПоле(string имя, Адрес адресТипаПоля) //адрес типа поля 
        {
            ...
            {
                [0] = new СтрокаЮникода("поле"),
                [1] = new СтрокаЮникода(имя),
                [2] = адресТипаПоля,
                [3] = new СтрокаЮникода("Значение")// это значение нужно только для объекта с полным самоописанием, значение поля экземпляра содержиться в экземпляре
            };
        ...
        public static ГруппаОбъектов СоздайЭкземплярКласса(ГруппаОбъектов класс)
        {
            ...
            экземпляр[0] = new СтрокаЮникода("экземпляр типа (адрес типа имя)");// это группа словосочетания 0=экземмпляр, 1 = вычисление (адрес типа- адрес имени типа)
        ...
        public static РАМОбъект ДайЗначение(ГруппаОбъектов экземпляр, string имяПоля)
        {
            var класс = (ГруппаОбъектов)экземпляр[0];
            for (int i = 1; i < ((ГруппаОбъектов)класс[2]).Список.Count; i++)
            {
                if (((СтрокаЮникода) ((ГруппаОбъектов) ((ГруппаОбъектов) класс[2])[i])[1]).Значение == имяПоля)
                    return экземпляр[i];
            }
            return new Пустота();
        ...
        public static РАМОбъект НаЗначение(ГруппаОбъектов экземпляр, string имяПоля, РАМОбъект значение)
        {
            var класс = (ГруппаОбъектов)экземпляр[0];
            ...
            for (int i = 1; i < ((ГруппаОбъектов)класс[2]).Список.Count; i++)
            {
                if (((СтрокаЮникода) ((ГруппаОбъектов) ((ГруппаОбъектов) класс[2])[i])[1]).Значение == имяПоля)
                {
                    ...
                    if (типЭкземпляраЗначения == ТипПоля)
                    {
                        ...
                        return new СтрокаЮникода("Готово");
                    }
                    return new СтрокаЮникода("Несовпадение типов поля и значения");

                }
                   
            }
            return new СтрокаЮникода("Нет такого имени поля");
