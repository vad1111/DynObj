...
namespace Динамические
{
    ...
    public class ГруппаОбщая : Примитив
    {
        ...
        public void ДобавитПрямуюСвязь(МягкаяСвязь новаяСвязь, РАМОбъект имяОбранойСвязи)
        {
            ПрямыеСвязи.Add(новаяСвязь);
            dynamic объектПрямойСвязи = новаяСвязь.АдресОбъекта.АдресВКучеПамяти();
        ...
        public void СообщитьПодписчикамОбИзмененииСобственногоАдреса( Адрес новыйАдрес)
        {
            ...
            {
                dynamic объектОбратнойСвязи = связь.АдресОбъекта.АдресВКучеПамяти();
