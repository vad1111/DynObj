...
public class ТСПСвязьИсходящая : Связь
{
    ...
    public override РАМОбъект ВыполнитьКоманду(Команда команда)
    {
        ...
        if(клиент==null || !клиент.Connected)
        {
            ...
            if(рез==false) return new СтрокаЮникода("сервер не доступен"); // или запуск метода обработки 
        }
        ...
        var результат = РАМОбъект.Создать(поток);
...
public class Посредник : РАМОбъект
{
    ...
    public РАМОбъект Выполнить( Команда команда)
    {
        var команда2 = new Команда() { Имя = new СтрокаЮникода() { Значение = "ВыполниКоманду" } };
    ...
    {
        var команда = new Команда() { Имя = new СтрокаЮникода() { Значение = "ДайКоманды" } };
        return (ГруппаОбъектов) Выполнить(команда); 
    }

    public РАМОбъект ТипУдаленногоОбъекта; // это получается универсальный объект, первый запрос это тип объекта, команды могут приниматься и без знания типа
...
public class ХранительКопии : РАМОбъект
{
    public ТСПСвязьИсходящая ТспСвязь;
    ...
    public РАМОбъект АдресВКучеКопияУдаленногоОбъекта; //здесь точная копия удаленного объекта адресация = ТСП+ индекс+ внутренний адрес
    ...
    public РАМОбъект Выполнить(Команда команда) // схоже с получи сообщение в активности 
    {
        ПолучитьКопию();
