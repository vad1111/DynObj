...
namespace Динамические
{
    ...
    public class ЦелоеНаДиске: Примитив
    {
        ...
        static ЦелоеНаДиске()
        {
            РеакцииНаКоманды.Add(new РеакцияНаКоманду() {Имя = new Строка("ДайЗначение") , Инструкция = typeof(ЦелоеНаДиске).GetMethod("ДайЗначение") });
        }
        ...
        public  ПримитивИлиАдрес УстановитьСвязь() // это связь, которую надо закрывать
        {
    ...
    public class ЦелоеСервер : Примитив
    {
            ...
            //востановить объект без значения из потока Хранилища
        {
            ...
            ЦелоеНаДиске = (ЦелоеНаДиске)ПримитивИлиАдрес.Создать(читатель.BaseStream);
            //восстановить адрес в сети
            Слушатель = (ТСПСвязьВходящая) ПримитивИлиАдрес.Создать(читатель.BaseStream);
            Старт();
...
// Это можно заменить на обработать команду и просто переслать 
    ...
    public class ТСПСвязьВходящая: Адрес
    {
        ...
        public ПримитивИлиАдрес ОбслуживаемыйПРимитив; //исходящее соединение - для активности туда посылаются команды 
        ...
        public void ЗапуститьСервер()
        {
            ...
            while (Активен) //цикл обработки подключений
            {
                ...
                ПримитивИлиАдрес результат= new Строка() {Значение = "ошибка"};
        ...
        {
            //
            var объект = ПримитивИлиАдрес.Создать(поток); // связь может застрять посредине чтения , если зависание больше нормы разорвать соединение 
