...
namespace Динамические
{
    ...
    public class План : ПримитивИлиАдрес
    {
        public Строка Имя;
        public ГруппаОбъектов Описание;
        public Процессор ТипПроцессора;  // Реальный(native),
        //public List<object> Параметры; //только значения параметров без описания параметры = группа параметров, параметр { имя ХХХ, тип ХХХ}  
        public ПримитивИлиАдрес РезультатДействия; //место откуда можно забрать результат
        // куда (это АдресВХранилище) направить результат или кому Активность.Получи(результат)
    ...
    // это действие рализует любую static команду С# 
    ...
    public class ПоследовательностьДействий: План //это блок кода - не функция или функция без параметров и возвращаемого результата
    {
        ...
        public override object Выполнить(План вызывающееДействие)
        {
            ...
            foreach (var действие in СписокДействий) // так не возможны возвраты повторы действий, да и хер ними без goto
            {
                if(действие is Прервать) { return "прервать";}
                if (действие is Продолжить) { return "продолжить"; }
                //if (действие is ВернутьРезультат) { return действие.Выполнить(this); } // просто в блоке вернуть не используется 
                try
                {
    ...
    public class ВызватьФункцию : План
    {
        ...
        public Dictionary<string, ПримитивИлиАдрес> СписокИменованныхПараметров; // если указывать параметры в виде списка (неименованного), то потребуется Список Параметров из Определения функции
    ...
    {
        public ПримитивИлиАдрес ВозращаемыйРезультат; // может быть и просто значение
        public override object Выполнить(План вызывающееДействие)
        {
    ...
    {
        public string ИмяПеременной;
        public ПримитивИлиАдрес Значение;
        public override object Выполнить(План вызывающееДействие)
        {
...
проверкаУсловия: значениеУсловия = (bool) УсловиеВыхода.Выполнить(this);
    ...
    public class Сложение : План
    {
        ...
        public ПримитивИлиАдрес слогаемое1;
        public ПримитивИлиАдрес слогаемое2;

        
        public void ИзменитьПараметры (АдресВХранилище параметры)
        {
            РезультатДействия = null;
            //Параметры = параметры;
        }
        ...
        private ЦелоеЧисло ПреобразоватьСлогаемое(План вызывающееДействие, ПримитивИлиАдрес параметр)
        {
            if (параметр is План)
            {
                return (ЦелоеЧисло) ((План) слогаемое1).Выполнить(this);
            }
            else if (слогаемое1 is Строка) // это должно быть действие ПолучитьЗначениеПеременной
            {
                return (ЦелоеЧисло) вызывающееДействие.СловарьВнутреннихПеременных[((Строка) параметр).Значение];
            }
            else
            {
    ...
    public class Значение : ПримитивИлиАдрес
    {
        private АдресВХранилище АВХ;

        public object ДайЗначение()
        {
            return АВХ.АдресВКуче();
        }
