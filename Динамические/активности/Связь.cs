...
namespace Динамические
{
    ...
    public class Связь: ПримитивИлиАдрес
    {
        public Канал Канал;

        public Активность РоднаяАктивность;
        public ПримитивИлиАдрес ОбслуживаемыйПРимитив; // это может быть и активность родная 
        ...
        public string Права;public bool ПравоНаОтдачуКоманд;
        ...
        public void Прием() // 
        {
            try
            {
                ...
                if (команда == null)
                {
                    Канал.ПередайИсходящееСообщение(new Строка("ошибка, ожидается команда"));
        ...
        public virtual ПримитивИлиАдрес УпаковатьСообщение(ПримитивИлиАдрес сообщение)
        {
        ...
        public virtual ПримитивИлиАдрес РаспаковатьСообщение(ПримитивИлиАдрес сообщение)
        {
            var команда = сообщение as Команда;
        ...
        public virtual void ПолучиСообщение(ПримитивИлиАдрес сообщение)
        {
            РоднаяАктивность.ПолучиСообщение(this, сообщение);
        }
        public virtual ПримитивИлиАдрес ПолучиСообщениеСОтветом(ПримитивИлиАдрес сообщение)
        {
            var ответ=РоднаяАктивность.ПолучиСообщениеСОтветом(this, сообщение);
            return ответ;
        }

        private void ОбработкаКоманды(ПримитивИлиАдрес сообщение) // это получи собщение с ответом 
        {
            var команда = сообщение as Команда; 
        ...
        private ПримитивИлиАдрес РезультатТекущейКоманды;
        void ПередачаОтвета()
        {
        ...
        private void ОбработкаОтветНаКоманду(ПримитивИлиАдрес сообщение)
        {
        ...
        private void ОбработкаСлужебнойКоманды(ПримитивИлиАдрес сообщение)
        {
            var команда = сообщение as Команда; 
        ...
        private void ПрямаяПередачаВАктивностьСОтветами() //вся логика в Активности -это реакция = действие ответное (это значит что инициатор внешний) на сообщения
        {
            try
            {
                ...
                var ответ = (ПримитивИлиАдрес)РоднаяАктивность.ПолучиСообщениеСОтветом(this,сообщение);
                // это аналог GET HTTP
                Канал.ПередайИсходящееСообщение(ответ);
            }
            catch (Exception e)
        ...
        private ПримитивИлиАдрес ТекущееИсходящееСообщение;

        public void ПередайСообщениеВСвоемПотоке(ПримитивИлиАдрес сообщение)
        {
